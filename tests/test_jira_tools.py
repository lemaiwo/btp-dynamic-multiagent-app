...
def test_toolset_build() -> None:
    ...
    names = _tool_names(_build({"destination": "MY_JIRA_DESTINATION",
                                "project": "ABC"}))
...
def test_storage_and_validation() -> None:
    ...
    payload = McpServerPayload(
        ...
        oauth=OAuthClientPayload(
            destination="MY_JIRA_DESTINATION", project="ABC", status="Open"),
    )
    check("a well-formed destination server validates",
          payload.oauth.to_config()["destination"] == "MY_JIRA_DESTINATION",
    ...
    err = _sync_raises(lambda: McpServerPayload(
        url="builtin:jira", auth_mode="destination",
        oauth=OAuthClientPayload(
            destination="MY_JIRA_DESTINATION", client_id="nope")))
    ...
    err = _sync_raises(lambda: McpServerPayload(
        ...
        oauth=OAuthClientPayload(destination="MY_JIRA_DESTINATION")))
