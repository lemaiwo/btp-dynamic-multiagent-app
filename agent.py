"""
Pydantic AI Agent with SAP MCP Server (OAuth2 + Streamable HTTP)

Connects to a remote MCP server using OAuth 2.1 authentication flow
and exposes a web chat interface. Uses SAP AI Core Generative AI Hub
as the LLM provider.
...
# Configuration
# ---------------------------------------------------------------------------
# Base URL for OAuth metadata discovery
MCP_BASE_URL = (
    "https://infrabel-app-dev-cf-ai-sap-mcp-server.cfapps.eu20-001.hana.ondemand.com"
)
...
oauth_provider = OAuthClientProvider(
    ...
    client_metadata=OAuthClientMetadata(
        client_name="Pydantic AI MCP Agent",
...
# ---------------------------------------------------------------------------
# MCP server connection (Streamable HTTP + OAuth2)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# SAP AI Core LLM (via Generative AI Hub)
# ---------------------------------------------------------------------------
...
model = SAPAICoreModel(
    "gpt-4o",  # model name as deployed in SAP AI Core
...
agent = Agent(
    model,
    instructions=(
        "You are a helpful assistant connected to an SAP MCP server. "
...
if __name__ == "__main__":
    ...
    print("Starting Pydantic AI Chat on http://127.0.0.1:7932")
